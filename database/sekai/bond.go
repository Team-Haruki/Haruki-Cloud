// Code generated by ent, DO NOT EDIT.

package sekai

import (
	"fmt"
	"haruki-cloud/database/sekai/bond"
	"strings"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
)

// Bond is the model entity for the Bond schema.
type Bond struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// ServerRegion holds the value of the "server_region" field.
	ServerRegion string `json:"server_region,omitempty"`
	// GameID holds the value of the "game_id" field.
	GameID int64 `json:"game_id,omitempty"`
	// GroupID holds the value of the "group_id" field.
	GroupID int64 `json:"group_id,omitempty"`
	// CharacterId1 holds the value of the "character_id1" field.
	CharacterId1 int64 `json:"character_id1,omitempty"`
	// CharacterId2 holds the value of the "character_id2" field.
	CharacterId2 int64 `json:"character_id2,omitempty"`
	selectValues sql.SelectValues
}

// scanValues returns the types for scanning values from sql.Rows.
func (*Bond) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case bond.FieldID, bond.FieldGameID, bond.FieldGroupID, bond.FieldCharacterId1, bond.FieldCharacterId2:
			values[i] = new(sql.NullInt64)
		case bond.FieldServerRegion:
			values[i] = new(sql.NullString)
		default:
			values[i] = new(sql.UnknownType)
		}
	}
	return values, nil
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the Bond fields.
func (_m *Bond) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case bond.FieldID:
			value, ok := values[i].(*sql.NullInt64)
			if !ok {
				return fmt.Errorf("unexpected type %T for field id", value)
			}
			_m.ID = int(value.Int64)
		case bond.FieldServerRegion:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field server_region", values[i])
			} else if value.Valid {
				_m.ServerRegion = value.String
			}
		case bond.FieldGameID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_id", values[i])
			} else if value.Valid {
				_m.GameID = value.Int64
			}
		case bond.FieldGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field group_id", values[i])
			} else if value.Valid {
				_m.GroupID = value.Int64
			}
		case bond.FieldCharacterId1:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field character_id1", values[i])
			} else if value.Valid {
				_m.CharacterId1 = value.Int64
			}
		case bond.FieldCharacterId2:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field character_id2", values[i])
			} else if value.Valid {
				_m.CharacterId2 = value.Int64
			}
		default:
			_m.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the Bond.
// This includes values selected through modifiers, order, etc.
func (_m *Bond) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this Bond.
// Note that you need to call Bond.Unwrap() before calling this method if this Bond
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *Bond) Update() *BondUpdateOne {
	return NewBondClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the Bond entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *Bond) Unwrap() *Bond {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("sekai: Bond is not a transactional entity")
	}
	_m.config.driver = _tx.drv
	return _m
}

// String implements the fmt.Stringer.
func (_m *Bond) String() string {
	var builder strings.Builder
	builder.WriteString("Bond(")
	builder.WriteString(fmt.Sprintf("id=%v, ", _m.ID))
	builder.WriteString("server_region=")
	builder.WriteString(_m.ServerRegion)
	builder.WriteString(", ")
	builder.WriteString("game_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameID))
	builder.WriteString(", ")
	builder.WriteString("group_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.GroupID))
	builder.WriteString(", ")
	builder.WriteString("character_id1=")
	builder.WriteString(fmt.Sprintf("%v", _m.CharacterId1))
	builder.WriteString(", ")
	builder.WriteString("character_id2=")
	builder.WriteString(fmt.Sprintf("%v", _m.CharacterId2))
	builder.WriteByte(')')
	return builder.String()
}

// Bonds is a parsable slice of Bond.
type Bonds []*Bond
