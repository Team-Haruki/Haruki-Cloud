...
func NewLocalFileService(sekaiClient *sekaiDB.Client, assetHelper *assets.AssetHelper, cfg LocalFileConfig) *Service {
	...
	if strings.TrimSpace(cfg.MusicMetaJSON) != "" {
		...
		service.musicMetaBytes = injectOmakaseMusicMeta(musicMetaBytes)
	}
...
func accumulateFloat(target map[string]interface{}, source map[string]interface{}, key string) {
...
func accumulateFloatSlice(target map[string]interface{}, source map[string]interface{}, key string) {
