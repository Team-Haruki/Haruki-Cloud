// Code generated by ent, DO NOT EDIT.

package sekai

import (
	"fmt"
	"haruki-cloud/database/sekai/mysekaigamecharacterunitgroup"
	"strings"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
)

// Mysekaigamecharacterunitgroup is the model entity for the Mysekaigamecharacterunitgroup schema.
type Mysekaigamecharacterunitgroup struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// GameID holds the value of the "game_id" field.
	GameID int `json:"game_id,omitempty"`
	// GameCharacterUnitId1 holds the value of the "game_character_unit_id1" field.
	GameCharacterUnitId1 int `json:"game_character_unit_id1,omitempty"`
	// GameCharacterUnitId2 holds the value of the "game_character_unit_id2" field.
	GameCharacterUnitId2 int `json:"game_character_unit_id2,omitempty"`
	// GameCharacterUnitId3 holds the value of the "game_character_unit_id3" field.
	GameCharacterUnitId3 int `json:"game_character_unit_id3,omitempty"`
	// GameCharacterUnitId4 holds the value of the "game_character_unit_id4" field.
	GameCharacterUnitId4 int `json:"game_character_unit_id4,omitempty"`
	// GameCharacterUnitId5 holds the value of the "game_character_unit_id5" field.
	GameCharacterUnitId5 int `json:"game_character_unit_id5,omitempty"`
	// ServerRegion holds the value of the "server_region" field.
	ServerRegion string `json:"server_region,omitempty"`
	selectValues sql.SelectValues
}

// scanValues returns the types for scanning values from sql.Rows.
func (*Mysekaigamecharacterunitgroup) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case mysekaigamecharacterunitgroup.FieldID, mysekaigamecharacterunitgroup.FieldGameID, mysekaigamecharacterunitgroup.FieldGameCharacterUnitId1, mysekaigamecharacterunitgroup.FieldGameCharacterUnitId2, mysekaigamecharacterunitgroup.FieldGameCharacterUnitId3, mysekaigamecharacterunitgroup.FieldGameCharacterUnitId4, mysekaigamecharacterunitgroup.FieldGameCharacterUnitId5:
			values[i] = new(sql.NullInt64)
		case mysekaigamecharacterunitgroup.FieldServerRegion:
			values[i] = new(sql.NullString)
		default:
			values[i] = new(sql.UnknownType)
		}
	}
	return values, nil
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the Mysekaigamecharacterunitgroup fields.
func (_m *Mysekaigamecharacterunitgroup) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case mysekaigamecharacterunitgroup.FieldID:
			value, ok := values[i].(*sql.NullInt64)
			if !ok {
				return fmt.Errorf("unexpected type %T for field id", value)
			}
			_m.ID = int(value.Int64)
		case mysekaigamecharacterunitgroup.FieldGameID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_id", values[i])
			} else if value.Valid {
				_m.GameID = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldGameCharacterUnitId1:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_character_unit_id1", values[i])
			} else if value.Valid {
				_m.GameCharacterUnitId1 = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldGameCharacterUnitId2:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_character_unit_id2", values[i])
			} else if value.Valid {
				_m.GameCharacterUnitId2 = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldGameCharacterUnitId3:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_character_unit_id3", values[i])
			} else if value.Valid {
				_m.GameCharacterUnitId3 = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldGameCharacterUnitId4:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_character_unit_id4", values[i])
			} else if value.Valid {
				_m.GameCharacterUnitId4 = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldGameCharacterUnitId5:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field game_character_unit_id5", values[i])
			} else if value.Valid {
				_m.GameCharacterUnitId5 = int(value.Int64)
			}
		case mysekaigamecharacterunitgroup.FieldServerRegion:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field server_region", values[i])
			} else if value.Valid {
				_m.ServerRegion = value.String
			}
		default:
			_m.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the Mysekaigamecharacterunitgroup.
// This includes values selected through modifiers, order, etc.
func (_m *Mysekaigamecharacterunitgroup) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this Mysekaigamecharacterunitgroup.
// Note that you need to call Mysekaigamecharacterunitgroup.Unwrap() before calling this method if this Mysekaigamecharacterunitgroup
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *Mysekaigamecharacterunitgroup) Update() *MysekaigamecharacterunitgroupUpdateOne {
	return NewMysekaigamecharacterunitgroupClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the Mysekaigamecharacterunitgroup entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *Mysekaigamecharacterunitgroup) Unwrap() *Mysekaigamecharacterunitgroup {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("sekai: Mysekaigamecharacterunitgroup is not a transactional entity")
	}
	_m.config.driver = _tx.drv
	return _m
}

// String implements the fmt.Stringer.
func (_m *Mysekaigamecharacterunitgroup) String() string {
	var builder strings.Builder
	builder.WriteString("Mysekaigamecharacterunitgroup(")
	builder.WriteString(fmt.Sprintf("id=%v, ", _m.ID))
	builder.WriteString("game_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameID))
	builder.WriteString(", ")
	builder.WriteString("game_character_unit_id1=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameCharacterUnitId1))
	builder.WriteString(", ")
	builder.WriteString("game_character_unit_id2=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameCharacterUnitId2))
	builder.WriteString(", ")
	builder.WriteString("game_character_unit_id3=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameCharacterUnitId3))
	builder.WriteString(", ")
	builder.WriteString("game_character_unit_id4=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameCharacterUnitId4))
	builder.WriteString(", ")
	builder.WriteString("game_character_unit_id5=")
	builder.WriteString(fmt.Sprintf("%v", _m.GameCharacterUnitId5))
	builder.WriteString(", ")
	builder.WriteString("server_region=")
	builder.WriteString(_m.ServerRegion)
	builder.WriteByte(')')
	return builder.String()
}

// Mysekaigamecharacterunitgroups is a parsable slice of Mysekaigamecharacterunitgroup.
type Mysekaigamecharacterunitgroups []*Mysekaigamecharacterunitgroup
