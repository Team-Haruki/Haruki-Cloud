...
func TestResponseElectionCompletionRetainsShortDedupWindow(t *testing.T) {
	rig := newResponseElectionTestRig(t, 2, time.Second)
...
func TestResponseElectionAllowsDifferentParametersDuringCompletedWindow(t *testing.T) {
	server := miniredis.RunT(t)
	baseTime := time.Date(2026, time.July, 15, 19, 0, 0, 0, time.UTC)
	server.SetTime(baseTime)
	client := redis.NewClient(&redis.Options{Addr: server.Addr()})
	coordinator := NewResponseElectionCoordinator(context.Background(), client, time.Second)
	t.Cleanup(func() {
	...
	request := responseElectionTestRequest("different-parameters")
	first, err := coordinator.join(context.Background(), responseElectionRequest{Request: request, BotID: "bot-a"})
	if err != nil || first.role != responseElectionExecutor {
		t.Fatalf("first join = %+v, err=%v, want executor", first, err)
	}
	...
	changed := request
	changed.Server = "cn"
	second, err := coordinator.join(context.Background(), responseElectionRequest{Request: changed, BotID: "bot-b"})
	if err != nil || second.role != responseElectionExecutor {
		t.Fatalf("different-parameter join = %+v, err=%v, want executor", second, err)
	}
}
...
func TestResponseElectionDeferredHandoffReelectsBeforeCommit(t *testing.T) {
	...
	stateTTL, err := rig.clients[0].PTTL(context.Background(), responseElectionStateKey(identity)).Result()
	if err != nil || stateTTL < responseElectionCompletedTTL-time.Second || stateTTL > responseElectionCompletedTTL {
		t.Fatalf("completed handoff TTL = %s (%v), want approximately %s", stateTTL, err, responseElectionCompletedTTL)
	}
}
