...
func musicBoardMetric(row musicBoardRow, target, liveType string) float64 {
	...
	case "pt/time":
		return derefMusicBoardFloat(selectMusicBoardLiveValue(row, liveType, "pt_per_hour"))
	case "tps":
		return row.Tps
	case "time":
...
func selectMusicBoardLiveValue(row musicBoardRow, liveType, field string) *float64 {
	switch liveType {
	case "auto":
		switch field {
		case "score":
