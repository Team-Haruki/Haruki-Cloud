...
func (handlerEducationRegionValidator) GetUserProfile(server, userID string) (*sekaiapi.GetAnotherProfileResponse, error) {
...
func (p *handlerEducationSnapshotProviderStub) Resolve(_ context.Context, _ rendersnapshot.Selector, opts rendersnapshot.ResolveOptions) (rendersnapshot.Snapshot, error) {
	if opts.NeedMySekai {
...
func TestExecuteEducationPowerFallsBackToSuiteSnapshotWhenMySekaiSnapshotUnavailable(t *testing.T) {
	...
	snapshot := mustBridgeEducationSnapshot(t)
	provider := &handlerEducationSnapshotProviderStub{
