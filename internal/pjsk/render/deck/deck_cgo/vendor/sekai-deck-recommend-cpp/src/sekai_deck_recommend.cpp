...
struct PyDeckRecommendOptions {
    ...
    std::optional<std::vector<int>> exclude_cards;
    ...
    py::dict to_dict() const {
        ...
        if (exclude_cards.has_value())
            result["exclude_cards"] = exclude_cards.value();
    ...
    static PyDeckRecommendOptions from_dict(const py::dict& dict) {
        ...
        if (dict.contains("exclude_cards"))
            options.exclude_cards = dict["exclude_cards"].cast<std::vector<int>>();
