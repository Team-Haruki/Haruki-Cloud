...
// ResolveCustomRoomMusicList returns music lists grouped by event rate.
// For each rate, up to numPerRate musics matching that rate are returned.
func (c *Controller) ResolveCustomRoomMusicList(region string, eventRates []int, numPerRate int) (map[int][]map[string]interface{}, error) {
	...
	payload := c.loadMetaPayload(resolved.String())
	if payload == nil {
		return nil, fmt.Errorf("music meta data unavailable for region %s", resolved)
	}

	_, source, builder, err := c.resolveBuilder(region)
	if err != nil {
		return nil, err
	}

	rateSet := make(map[int]struct{}, len(eventRates))
	for _, rate := range eventRates {
		rateSet[rate] = struct{}{}
	}

	result := make(map[int][]map[string]interface{}, len(eventRates))
	for _, musicInfo := range source.GetMusics() {
		if musicInfo == nil {
			continue
		}
		...
		for _, meta := range metas {
			...
			if _, wanted := rateSet[rate]; !wanted {
				continue
			}
...
// ResolveMusicMetaRequests resolves music queries into MusicMetaRequest objects
// containing metadata (event rates, scores, etc.) for all difficulties.
func (c *Controller) ResolveMusicMetaRequests(region string, queries []string) ([]drawing.MusicMetaRequest, error) {
	...
	for _, query := range queries {
		music, searchErr := source.SearchMusic(strings.TrimSpace(query))
		if searchErr != nil || music == nil {
			continue
		}
		metas := musicMetaInfosFromPayload(payload, music.ID)
		results = append(results, drawing.MusicMetaRequest{
			MusicID:        music.ID,
			MusicTitle:     music.Title,
			MusicCoverPath: builder.BuildMusicJacketPath(music.AssetBundleName, resolved),
			Metas:          metas,
		})
	}
	return results, nil
...
func (c *Controller) loadMetaPayload(region string) []byte {
	if c == nil || c.metaLoader == nil {
		return nil
	}
	return c.metaLoader.Get(region)
}

// ResolveMusicBoardRequest builds a MusicBoardRequest from a BoardQuery,
// resolving music items from metadata and applying sorting/pagination.
func (c *Controller) ResolveMusicBoardRequest(region string, query BoardQuery) (*drawing.MusicBoardRequest, error) {
	...
	// Resolve spec queries to (musicID, difficulty) pairs
	var specMidDiffs [][]interface{}
	for _, sq := range query.SpecQueries {
		sq = strings.TrimSpace(sq)
		if sq == "" {
			continue
		}
		m, searchErr := source.SearchMusic(sq)
		if searchErr != nil || m == nil {
			continue
		}
		for _, diff := range []string{"easy", "normal", "hard", "expert", "master", "append"} {
			if builder.GetDifficultyLevel(m.ID, diff) > 0 {
				specMidDiffs = append(specMidDiffs, []interface{}{m.ID, diff})
			}
		}
	}

	// Build board items from all musics and their meta data
	diffs := query.DiffFilter
	if len(diffs) == 0 {
		diffs = []string{"easy", "normal", "hard", "expert", "master", "append"}
	}

	items := make([]drawing.MusicBoardItem, 0)
	now := time.Now().UnixMilli()
	for _, musicInfo := range source.GetMusics() {
		...
		for _, diff := range diffs {
			...
			if item.MusicTime > 0 {
				item.Tps = baseScore / item.MusicTime
			}
...
func sortMusicBoardItems(items []drawing.MusicBoardItem, target string, ascend bool) {
	sort.SliceStable(items, func(i, j int) bool {
		...
		switch target {
		case "event_rate":
			a, b = items[i].EventRate, items[j].EventRate
		case "music_time":
			a, b = items[i].MusicTime, items[j].MusicTime
		case "tps":
			a, b = items[i].Tps, items[j].Tps
		default:
			a, b = items[i].EventRate, items[j].EventRate
		}
		if ascend {
