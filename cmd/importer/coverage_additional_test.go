...
`, "file:"+pjskPath+"?_fk=1", "file:"+usersPath+"?_fk=1"))
		...
		if err != nil {
			t.Fatalf("open importer preflight DB: %v", err)
		}
		if err := preflight.Schema.Create(context.Background()); err != nil {
			t.Fatalf("migrate importer preflight DB: %v", err)
		}
		if err := preflight.Close(); err != nil {
			t.Fatalf("close importer preflight DB: %v", err)
		}
...
func TestImportBindingsStableDatabaseFailures(t *testing.T) {
	...
	t.Run("resolve user", func(t *testing.T) {
		...
		if err := importBindings(ctx, dir, pjsk, users, false); err != nil {
			t.Fatalf("resolve-user record failure should be skipped: %v", err)
		}
	})
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := importBindings(ctx, dir, pjsk, users, false); err != nil {
				t.Fatalf("record query failure should be skipped: %v", err)
			}
		})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := importBindings(ctx, dir, pjsk, users, false); err != nil {
				t.Fatalf("game-account create failure should be skipped: %v", err)
			}
		})
	}
	...
	t.Run("binding create", func(t *testing.T) {
		pjsk, users := openImporterTestClients(t)
		if _, err := pjsk.GameAccount.Create().SetServer("jp").SetUserID("20001").Save(ctx); err != nil {
			t.Fatalf("seed game account: %v", err)
		}
		installImporterMutationFailure(pjsk, forced)
		if err := importBindings(ctx, dir, pjsk, users, false); err != nil {
			t.Fatalf("binding create failure should be skipped: %v", err)
		}
	})
	...
	t.Run("binding constraint", func(t *testing.T) {
		...
		if err := importBindings(ctx, dir, pjsk, users, false); err != nil {
			t.Fatalf("binding constraint should be skipped: %v", err)
		}
	})
}
...
func TestAliasImportsStableDatabaseFailures(t *testing.T) {
	...
	runAll := func(t *testing.T, client *pjskDB.Client) {
		...
		for name, call := range map[string]func() error{
			...
			if err := call(); err != nil {
				t.Fatalf("%s record failure should be skipped: %v", name, err)
			}
		}
	}
...
func TestSetDefaultBindingsRemainingBranches(t *testing.T) {
	...
	t.Run("binding without account", func(t *testing.T) {
		client, _ := openImporterTestClients(t)
		if _, err := client.UserBinding.Create().SetHarukiUserID(1).Save(ctx); err != nil {
			t.Fatalf("create edge-less binding: %v", err)
		}
		if err := setDefaultBindings(ctx, client, false); err != nil {
			t.Fatalf("skip edge-less binding: %v", err)
		}
	})
	...
	t.Run("existing defaults query", func(t *testing.T) {
		...
		if err := setDefaultBindings(ctx, client, false); !errors.Is(err, forced) {
			t.Fatalf("existing-default query error = %v", err)
		}
	})
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := client.UserBinding.Create().SetHarukiUserID(1).SetGameAccountID(account.ID).Save(ctx); err != nil {
				t.Fatalf("create binding: %v", err)
			}
			installImporterMutationFailure(client, tc.forced)
			if err := setDefaultBindings(ctx, client, false); err != nil {
				t.Fatalf("default creation failures are per-record: %v", err)
			}
		})
	}
	...
	t.Run("context cancellation after loading", func(t *testing.T) {
		...
		if err := setDefaultBindings(cancelCtx, client, false); !errors.Is(err, context.Canceled) {
			t.Fatalf("post-load cancellation error = %v", err)
		}
	})
}
...
func TestImportBindingsDryRunProgressBranch(t *testing.T) {
	...
	records = append(records, ']')
	if err := os.WriteFile(filepath.Join(dir, "bind.json"), records, 0o600); err != nil {
		t.Fatalf("write large dry-run fixture: %v", err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := importBindings(ctx, dir, nil, nil, true); err != nil {
		t.Fatalf("large dry-run import: %v", err)
	}
}
