...
> ⚠️ **所有 MySekai 指令需用户绑定 Toolbox 账号**（suite + mysekai 数据），且对 `region=cn` 关闭（除非在 `allow_cn_mysekai`
> 白名单中）
...
时区说明：`/注册时间`、`/sud`、`/msd` 输出时间默认使用 **UTC+8**，用户可在 UserSettings 中通过 `TimeZoneOffset`（如 `+09:00`
）自定义时区。
