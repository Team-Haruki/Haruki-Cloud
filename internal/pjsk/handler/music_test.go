...
func TestFormatMusicBPMSequenceDoesNotTruncate(t *testing.T) {
	events := make([]rendermusic.BPMEvent, 0, 12)
	for i := 0; i < 12; i++ {
		events = append(events, rendermusic.BPMEvent{BPM: float64(100 + i)})
	}

	got := formatMusicBPMSequence(events)
	if strings.Contains(got, "…") {
		t.Fatalf("sequence was truncated: %q", got)
	}
	if !strings.Contains(got, "100") || !strings.Contains(got, "111") {
		t.Fatalf("sequence missing endpoints: %q", got)
	}
}
