...
func TestPreview3DResponsesHaveExplicitSizeLimits(t *testing.T) {
	engine := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var size int64
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/runtime/test.json":
	...
	if err := service.getJSON(context.Background(), endpoint, "/runtime/test.json", &registry); err == nil || !strings.Contains(err.Error(), "registry response exceeds") {
		t.Fatalf("getJSON oversized response error = %v", err)
	}
