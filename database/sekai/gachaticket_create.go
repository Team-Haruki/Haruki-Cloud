// Code generated by ent, DO NOT EDIT.

package sekai

import (
	"context"
	"errors"
	"fmt"
	"haruki-cloud/database/sekai/gachaticket"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
)

// GachaticketCreate is the builder for creating a Gachaticket entity.
type GachaticketCreate struct {
	config
	mutation *GachaticketMutation
	hooks    []Hook
}

// SetServerRegion sets the "server_region" field.
func (_c *GachaticketCreate) SetServerRegion(v string) *GachaticketCreate {
	_c.mutation.SetServerRegion(v)
	return _c
}

// SetGameID sets the "game_id" field.
func (_c *GachaticketCreate) SetGameID(v int64) *GachaticketCreate {
	_c.mutation.SetGameID(v)
	return _c
}

// SetNillableGameID sets the "game_id" field if the given value is not nil.
func (_c *GachaticketCreate) SetNillableGameID(v *int64) *GachaticketCreate {
	if v != nil {
		_c.SetGameID(*v)
	}
	return _c
}

// SetName sets the "name" field.
func (_c *GachaticketCreate) SetName(v string) *GachaticketCreate {
	_c.mutation.SetName(v)
	return _c
}

// SetNillableName sets the "name" field if the given value is not nil.
func (_c *GachaticketCreate) SetNillableName(v *string) *GachaticketCreate {
	if v != nil {
		_c.SetName(*v)
	}
	return _c
}

// SetAssetbundleName sets the "assetbundle_name" field.
func (_c *GachaticketCreate) SetAssetbundleName(v string) *GachaticketCreate {
	_c.mutation.SetAssetbundleName(v)
	return _c
}

// SetNillableAssetbundleName sets the "assetbundle_name" field if the given value is not nil.
func (_c *GachaticketCreate) SetNillableAssetbundleName(v *string) *GachaticketCreate {
	if v != nil {
		_c.SetAssetbundleName(*v)
	}
	return _c
}

// SetGachaDisplayType sets the "gacha_display_type" field.
func (_c *GachaticketCreate) SetGachaDisplayType(v string) *GachaticketCreate {
	_c.mutation.SetGachaDisplayType(v)
	return _c
}

// SetNillableGachaDisplayType sets the "gacha_display_type" field if the given value is not nil.
func (_c *GachaticketCreate) SetNillableGachaDisplayType(v *string) *GachaticketCreate {
	if v != nil {
		_c.SetGachaDisplayType(*v)
	}
	return _c
}

// Mutation returns the GachaticketMutation object of the builder.
func (_c *GachaticketCreate) Mutation() *GachaticketMutation {
	return _c.mutation
}

// Save creates the Gachaticket in the database.
func (_c *GachaticketCreate) Save(ctx context.Context) (*Gachaticket, error) {
	return withHooks(ctx, _c.sqlSave, _c.mutation, _c.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (_c *GachaticketCreate) SaveX(ctx context.Context) *Gachaticket {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *GachaticketCreate) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *GachaticketCreate) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}

// check runs all checks and user-defined validators on the builder.
func (_c *GachaticketCreate) check() error {
	if _, ok := _c.mutation.ServerRegion(); !ok {
		return &ValidationError{Name: "server_region", err: errors.New(`sekai: missing required field "Gachaticket.server_region"`)}
	}
	return nil
}

func (_c *GachaticketCreate) sqlSave(ctx context.Context) (*Gachaticket, error) {
	if err := _c.check(); err != nil {
		return nil, err
	}
	_node, _spec := _c.createSpec()
	if err := sqlgraph.CreateNode(ctx, _c.driver, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	id := _spec.ID.Value.(int64)
	_node.ID = int(id)
	_c.mutation.id = &_node.ID
	_c.mutation.done = true
	return _node, nil
}

func (_c *GachaticketCreate) createSpec() (*Gachaticket, *sqlgraph.CreateSpec) {
	var (
		_node = &Gachaticket{config: _c.config}
		_spec = sqlgraph.NewCreateSpec(gachaticket.Table, sqlgraph.NewFieldSpec(gachaticket.FieldID, field.TypeInt))
	)
	if value, ok := _c.mutation.ServerRegion(); ok {
		_spec.SetField(gachaticket.FieldServerRegion, field.TypeString, value)
		_node.ServerRegion = value
	}
	if value, ok := _c.mutation.GameID(); ok {
		_spec.SetField(gachaticket.FieldGameID, field.TypeInt64, value)
		_node.GameID = value
	}
	if value, ok := _c.mutation.Name(); ok {
		_spec.SetField(gachaticket.FieldName, field.TypeString, value)
		_node.Name = value
	}
	if value, ok := _c.mutation.AssetbundleName(); ok {
		_spec.SetField(gachaticket.FieldAssetbundleName, field.TypeString, value)
		_node.AssetbundleName = value
	}
	if value, ok := _c.mutation.GachaDisplayType(); ok {
		_spec.SetField(gachaticket.FieldGachaDisplayType, field.TypeString, value)
		_node.GachaDisplayType = value
	}
	return _node, _spec
}

// GachaticketCreateBulk is the builder for creating many Gachaticket entities in bulk.
type GachaticketCreateBulk struct {
	config
	err      error
	builders []*GachaticketCreate
}

// Save creates the Gachaticket entities in the database.
func (_c *GachaticketCreateBulk) Save(ctx context.Context) ([]*Gachaticket, error) {
	if _c.err != nil {
		return nil, _c.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(_c.builders))
	nodes := make([]*Gachaticket, len(_c.builders))
	mutators := make([]Mutator, len(_c.builders))
	for i := range _c.builders {
		func(i int, root context.Context) {
			builder := _c.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*GachaticketMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, _c.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, _c.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				mutation.id = &nodes[i].ID
				if specs[i].ID.Value != nil {
					id := specs[i].ID.Value.(int64)
					nodes[i].ID = int(id)
				}
				mutation.done = true
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, _c.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (_c *GachaticketCreateBulk) SaveX(ctx context.Context) []*Gachaticket {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *GachaticketCreateBulk) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *GachaticketCreateBulk) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}
