...
func TestExecuteMusicChartUsesBriefListForAmbiguousAlias(t *testing.T) {
	briefListCalls := 0
	chartCalls := 0
...
func TestExecuteMusicBPMDetailReturnsSongBPMText(t *testing.T) {
	root := t.TempDir()
	chartPath := filepath.Join(root, "music", "music_score", "0001_01", "expert.txt")
	if err := os.MkdirAll(filepath.Dir(chartPath), 0o755); err != nil {
		t.Fatalf("mkdir chart: %v", err)
	...
	params, err := json.Marshal(map[string]any{"difficulty": "expert"})
	if err != nil {
		t.Fatalf("marshal params: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("executeMusic bpm detail: %v", err)
	}
	if len(message) != 1 || message[0].Type != onebot11.TypeText {
		t.Fatalf("unexpected bpm detail message: %+v", message)
	}
	textData, ok := message[0].Data.(onebot11.TextData)
	if !ok {
		t.Fatalf("unexpected text data: %#v", message[0].Data)
	}
	if !strings.Contains(textData.Text, "【1】Song A") ||
		!strings.Contains(textData.Text, "难度：EXPERT") ||
		!strings.Contains(textData.Text, "主 BPM：120") ||
		!strings.Contains(textData.Text, "BPM 变化：120 / 180") {
		t.Fatalf("unexpected bpm detail text: %q", textData.Text)
	}
}

func TestExecuteMusicBPMDetailUsesBriefListForAmbiguousAlias(t *testing.T) {
	briefListCalls := 0
	titles := make([]string, 0, 1)
	drawingServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/pjsk/music/brief-list":
			...
			if req.MusicList[0].ID != 1 || req.MusicList[1].ID != 2 {
				t.Fatalf("unexpected ambiguous bpm ids: %+v", req.MusicList)
			}
			_, _ = w.Write([]byte("png"))
		default:
			t.Fatalf("unexpected drawing path: %s", r.URL.Path)
	...
	if len(titles) != 1 || titles[0] != "匹配到多个歌曲，请使用 /查BPM <id> 查询：" {
		t.Fatalf("unexpected title list: %+v", titles)
	}
}
...
func TestResolveDeckCharacterSelectionsFallsBackUnknownWorldBloomQueryToMusicAndFirstChapter(t *testing.T) {
	ctx := context.Background()
	...
	if query.WorldBloomCharacterID == nil || *query.WorldBloomCharacterID != 21 {
		t.Fatalf("expected fallback to first world bloom chapter character, got %+v", query.WorldBloomCharacterID)
	}
	if query.WorldBloomCharacterQuery != "" {
		t.Fatalf("expected world bloom character query to be cleared, got %q", query.WorldBloomCharacterQuery)
	}
	if query.MusicQuery != "虾" {
		t.Fatalf("expected unknown wl query to fallback to music query, got %q", query.MusicQuery)
	}
}
...
func TestExecuteProfileCustomProfileCard(t *testing.T) {
	sekaiServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/jp/77777777777777/profile" {
			t.Fatalf("unexpected sekai path: %s", r.URL.Path)
		}
		_ = json.ConfigDefault.NewEncoder(w).Encode(sekaiapi.GetAnotherProfileResponse{
			User: sekaiapi.AnotherUser{
				UserID: 77777777777777,
				Name:   "Tester",
			},
			UserProfile: sekaiapi.UserProfile{Word: "hello"},
			UserCustomProfileCards: []sekaiapi.UserCustomProfileCard{
				{
					CustomProfileID:     1,
					CustomProfileCardID: 1,
					Seq:                 1,
					ThumbnailPath:       "legacy-thumbnail-path",
	...
	oldBaseURL := config.Cfg.SekaiAPI.BaseURL
	oldToken := config.Cfg.SekaiAPI.Token
	config.Cfg.SekaiAPI.BaseURL = sekaiServer.URL
	config.Cfg.SekaiAPI.Token = "test-token"
	t.Cleanup(func() {
		config.Cfg.SekaiAPI.BaseURL = oldBaseURL
		config.Cfg.SekaiAPI.Token = oldToken
	...
	if _, err := service.Bind(context.Background(), "qq", "42", "77777777777777"); err != nil {
		t.Fatalf("bind account: %v", err)
	}
	app := &renderapp.App{
		Bindings:   service,
		Drawing:    drawing.NewHarukiDrawingClient(drawingServer.URL),
		ImageCache: imagecache.New("https://image-cache.test", t.TempDir()),
		SekaiAPI:   sekaiapi.NewSekaiAPIClient(&config.Cfg.SekaiAPI),
	}
	...
	message, err := executeProfile(NewRequestContext(context.Background(), &CommandRequest{
		Module:            parser.ModuleProfile,
		Mode:              profileModeCustomProfileCard,
		Region:            "jp",
		Params:            params,
		RequesterPlatform: "qq",
		RequesterUserID:   "42",
	...
	if err != nil {
		t.Fatalf("executeProfile custom profile: %v", err)
	}
	if len(message) != 1 || message[0].Type != "image" {
		t.Fatalf("unexpected message: %+v", message)
	}
...
func TestExecuteProfileCustomProfileCardSelectsSeq(t *testing.T) {
	sekaiServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/jp/88888888888888/profile" {
			t.Fatalf("unexpected sekai path: %s", r.URL.Path)
		}
		_ = json.ConfigDefault.NewEncoder(w).Encode(sekaiapi.GetAnotherProfileResponse{
			User: sekaiapi.AnotherUser{
	...
	service := newHandlerTestBindingServiceWithValidator(t, handlerTestBindingValidator{})
	if _, err := service.Bind(context.Background(), "qq", "42", "88888888888888"); err != nil {
		t.Fatalf("bind account: %v", err)
	}
	app := &renderapp.App{
		Bindings:   service,
	...
	if err != nil {
		t.Fatalf("executeProfile custom profile seq: %v", err)
	}
	if len(message) != 1 || message[0].Type != "image" {
		t.Fatalf("unexpected message: %+v", message)
	}
