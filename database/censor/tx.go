...
func (tx *Tx) init() {
	tx.ImageModCache = NewImageModCacheClient(tx.config)
	tx.NameLog = NewNameLogClient(tx.config)
	tx.Result = NewResultClient(tx.config)
	tx.ShortBio = NewShortBioClient(tx.config)
}
...
// applies a query, for example: ImageModCache.QueryXXX(), the query will be executed
